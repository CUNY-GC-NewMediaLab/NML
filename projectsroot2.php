<?php
/*
Template Name: Projects Root Page 2
*/
?>
<?php 
...
<h2>Current Projects</h2>
	...
	foreach ($pages as $pagg) {
  		...
  		$studentnameurl	=	get_post_meta($pagg->ID, 'studentnameurl ', false);
		...
		$gdesc	=	strip_tags($rdesc[0]);
  		...
  		$option .=  '<a href="' .get_page_link($pagg->ID)   .  '"><img src="' . $simage[0] . '"></a><p class="proj-desc">'. $rdesc[0] .'</p><p class="proj-auth"><a href="' . $studentnameurl . '">'. $studentname[0] ."</a>, ". $depo[0] . '</p></div>';
  		$option .= '</li>'; 
		echo $option;	
 	}
 	echo "</ul>";
...
<h2>Previous Projects</h2>
	...
	foreach ($pages as $pagg) {
		...
		$gdesc	=	strip_tags($rdesc[0]);
  		$option .= '<div style="display:none">';	
  		$option .=  '<a href="' .get_page_link($pagg->ID)   .  '"><img src="' . $simage[0] . '"></a><p class="proj-desc">'. $rdesc[0] .'</p><p class="proj-auth">'. $studentname[0] .", ". $depo[0] . '</p></div>';
  		$option .= '</li>'; 
		echo $option;	
 	}
 	echo "</ul>";
